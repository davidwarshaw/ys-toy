...
export default class GameScene extends Phaser.Scene {
  ...
  createBackgroundLayers() {
    this.layers.background = this.map.createStaticLayer("background", this.tileset);
    this.layers.collision = this.map.createStaticLayer("collision", this.tileset);
    this.map.setCollisionByExclusion([-1], true, true, this.layers.collision);
  }

  createForegroundLayer() {
    this.layers.foreground = this.map.createStaticLayer("foreground", this.tileset);
  }
